    ...
    return adminApi
      ...
      .then(resp => {
        ...
        return Promise.resolve()
          .then(
            () =>
              !callbackConfig.canCreateTresor ||
              callbackConfig.canCreateTresor(cUser, members, tresorId)
          )
          .then(appApproved => {
            if (!appApproved) {
          ...
          .then(() => {
            const tresor = Tresor.create({ id: tresorId, members: members });
            debugLog("Saved %o", tresor);
            return tresor;
          });
      });
  }
    ...
    return adminApi
      ...
      .then(details => {
        ...
        return Promise.all([
          ...
          User.findOne({ zkitId: inviterId }),
          User.findOne({ zkitId: inviteeId })
  ...
  function kickedUser(operationId, cUser) {
    ...
    return adminApi
      ...
      .then(details => {
        ...
        return Promise.all([
          Tresor.findOne({ id: tresorId }),
          User.findOne({ zkitId: kickerId }),
          User.findOne({ zkitId: kickedId })
