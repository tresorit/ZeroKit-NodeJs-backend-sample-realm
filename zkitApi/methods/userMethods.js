...
module.exports = function(callbackConfig) {
  ...
  function initUserReg(userName, profileData) {
    ...
    return User.findOne({ userName: userName })
      .then(user => {
        if (user) {
          if (user.state > 0) throw errors.badInput("UserNameTaken");
          else return user.remove(); // TODO: handle simultaneous regs
        }
      })
      .then(() => {
  ...
  function finishUserReg(userId, userVerifier) {
    ...
    return User.findOne({ zkitId: userId, state: 0 }).then(user => {
      ...
      return Promise.resolve()
        .then(
          () =>
            !callbackConfig.canFinishRegistration ||
            callbackConfig.canFinishRegistration(user.toObject())
        )
        .then(appApproved => {
          if (!appApproved) throw errors.forbidden("ApplicationDenied");
        ...
        .then(
          () =>
            !callbackConfig.finishedRegistration ||
            callbackConfig.finishedRegistration(user.toObject(), validationCode)
        );
    });
  }
  ...
  function validateUser(userId, validationCode) {
    ...
    return User.findOne({ zkitId: userId }).then(user => {
      ...
      if (user.registrationData.validationCode !== validationCode)
        throw errors.forbidden("InvalidValidationCode");

      return Promise.resolve() // This way sync and async errors are captured the same way.
        .then(
          () => !callbackConfig.canValidateUser || callbackConfig.canValidateUser(user.toObject())
        )
        .then(appApproved => {
          if (!appApproved) throw errors.forbidden("ApplicationDenied");
