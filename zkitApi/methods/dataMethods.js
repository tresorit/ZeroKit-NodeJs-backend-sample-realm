...
module.exports = function(callbackConfig) {
  ...
  function getPublicProfile(userId) {
    return User.findOne({ zkitId: userId }).then(user => {
      if (!user) throw errors.notFound("UserNotFound");
  ...
  function storeProfile(cUser, data) {
    return User.findOne({ zkitId: cUser.zkitId }).then(user => {
      ...
      return Promise.resolve()
        .then(
          () =>
            !callbackConfig.canStoreProfile ||
            callbackConfig.canStoreProfile(user.toObject(), data, cUser)
        )
        .then(appApproved => {
          if (!appApproved) throw errors.forbidden("ApplicationDenied");
          debugLog("App approved");
          return callbackConfig.transformProfileToStore
            ? callbackConfig.transformProfileToStore(user, data)
            : data;
        })
        .then(data => {
  ...
  function storePublicProfile(cUser, data) {
    return User.findOne({ zkitId: cUser.zkitId }).then(user => {
      ...
      return Promise.resolve()
        .then(
          () =>
            !callbackConfig.canStorePublicProfile ||
            callbackConfig.canStorePublicProfile(user.toObject(), data, cUser)
        )
        .then(appApproved => {
          if (!appApproved) throw errors.forbidden("ApplicationDenied");
          debugLog("App approved");
          return callbackConfig.transformPublicProfileToStore
            ? callbackConfig.transformPublicProfileToStore(user, data)
            : data;
        })
        .then(data => {
  ...
  function getData(dataId, cUser) {
    ...
    return Data.findOne({ id: dataId }).then(entry => {
      ...
      return Promise.resolve()
        ...
        .then(appApproved => {
          ...
          return callbackConfig.transformDataToGet
            ? callbackConfig.transformDataToGet(entry)
            : entry.data;
        })
        .then(data => {
  ...
  function storeData(dataId, tresorId, cUser, data) {
    debugLog("%s requested to store data by id %s for tresor %s", cUser.zkitId, dataId, tresorId);
    return Promise.all([Tresor.findOne({ id: tresorId }), Data.findOne({ id: dataId })]).then(([
      tresor,
      entry
    ]) => {
      if (entry) debugLog("An entry already exists:\n%O", entry);
      return Promise.resolve(
        !callbackConfig.canStoreData ||
          callbackConfig.canStoreData(cUser, dataId, data, tresor.toObject(), entry === null ? null : entry.toObject())
      )
        .then(hasAppAccess => {
          ...
          return callbackConfig.transformDataToStore
            ? callbackConfig.transformDataToStore(
                cUser,
                dataId,
                data,
                entry ? entry.toObject() : null
              )
            : data;
        })
        .then(data => {
          if (!entry) {
            return Data.create({ id: dataId }).then(newEntry => {
              newEntry.tresor = tresor;
              newEntry.data = data;
              debugLog("Saving %o", newEntry);
