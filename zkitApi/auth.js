...
function init(callbackConfig) {
  ...
  config.zeroKit.idp.map(client => {
    ...
    passport.use(
      ...
      new passport_openidconnect.Strategy(client, openidVerifyCallback)
    );

    const currClient = new OAuth2(
      client.clientID,
      client.clientSecret,
      "",
    ...
    ouathClient.getOAuthAccessToken(
      ...
      function(err, access_token, refresh_token, results) {
        ...
        */
        ...
        if (idtoken.payload.exp * 1000 < Date.now()) next(errors.forbidden("Expired token"));
