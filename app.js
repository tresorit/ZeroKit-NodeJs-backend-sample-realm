...
app.use(
  dnsRebindingProtection(["localhost", "127.0.0.1", /^https?:\/\/(.*)/.exec(config.baseUrl)[1]])
);
...
const callbackConfig = {
  ...
  initReg: function(userName, profileData) {
    if (isTestUser({ userName })) return checkProfileProp(profileData, "canInitReg", true);
    return true;
  },
  ...
  finishedRegistration: function(user, validationCode) {
    if (isTestUser(user) && checkProfileProp(user.profileData, "autoValidate", false)) {
      return zkitApi.user.methods.validateUser(user.zkitId, validationCode).then(() => true);
    }
  ...
  canCreateTresor: function(user, tresorId) {
    if (isTestUser(user)) return checkProfileProp(user.profileData, "canCreateTresor", true);
    return true;
  },

  approveShare: function(tresor, invitee, invited, user) {
    if (isTestUser(user)) return checkProfileProp(user.profileData, "canShare", true);
    return true;
  },

  approveKick: function(tresor, kicker, kicked, user) {
    if (isTestUser(user)) return checkProfileProp(user.profileData, "canKick", true);
    return true;
  },

  canAccessData: function(user, entry) {
    let testVal;
    if (isTestUser(user)) testVal = checkProfileProp(user.profileData, "canAccessData", null);
  ...
  canStoreData: function(user, dataid, body, tresor, oldEntry) {
    let testVal;
    if (isTestUser(user)) testVal = checkProfileProp(user.profileData, "canStoreData", null);
    return testVal === null
      ? !oldEntry || oldEntry.tresor.members.indexOf(user.zkitId) !== -1
      : testVal;
  },
  ...
  canStoreProfile: function(user, profileData) {
    if (isTestUser(user)) return checkProfileProp(user.profileData, "canStoreProfile", true);
    return true;
  },
